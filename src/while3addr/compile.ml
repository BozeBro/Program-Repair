...
let compile stmt =
  ...
  and transform_cond prog = function
    ...
    | While.LT (Var z, Num 0) ->
        (* a lil special casing... *)
        (z, LT, prog)
    | While.LT (a1, Num 0) ->
        let prog, v1 = transform_aexp prog a1 in
        (v1, LT, prog)
    ...
    | While.LEQ (a1, a2) -> transform_cond prog (Or (LT (a1, a2), EQ (a1, a2)))
    | While.GEQ (a1, a2) -> transform_cond prog (Or (GT (a1, a2), EQ (a1, a2)))
    | While.And (b1, b2) ->
        (* I know this is hideous, hard to keep all the pieces in my head *)
        (* basically, we do:
           if (b1 and b2) -->
           0. goto 5
           1. a0 = 0
        ...
        *)
        ...
        let jump_to_if prog pc =
          insert_in_program prog goto_start_of_if (Goto pc)
        in
        let thenstart = goto_start_of_if + 1 in
        let jump_to_end_of_else, prog =
          append_to_program (thenstart, prog) (ConstAssign (ares, 0))
        in
        let elsepc, prog =
          append_to_program
            (jump_to_end_of_else, prog)
            (Goto (jump_to_end_of_else + 2))
        in
        let jump_to_end_of_if, prog =
          append_to_program (elsepc, prog) (ConstAssign (ares, 1))
        in
        let jump_over_if prog pc =
          insert_in_program prog jump_to_end_of_if (Goto pc)
        in
        let b1v, op1, (start_of_if, prog) =
          transform_cond (jump_to_end_of_if + 1, prog) b1
        in
        let end_of_b1_if, prog =
          append_to_program (start_of_if, prog)
            (IfGoto (b1v, op1, start_of_if + 2))
        in
        let nextcond, prog =
          append_to_program (end_of_b1_if, prog) (Goto elsepc)
        in
        let b2v, op2, (b2pc, prog) = transform_cond (nextcond, prog) b2 in
        let ifpc, prog =
          append_to_program (b2pc, prog) (IfGoto (b2v, op2, thenstart))
        in
    ...
    | While.Or (b1, b2) ->
        ...
        in
        let prog =
          append_to_program (start_of_if, prog) (IfGoto (b1v, op1, thenstart))
        in
  ...
  and transform_stmt prog = function
    ...
    | While.ArrAssign (x, Var y) ->
        append_to_program prog (VarAssignArray (x, y))
    | While.ArrAssign (x, Num y) ->
        append_to_program prog (ConstAssignArray (x, y))
    ...
    | While.If (b, s1, s2) ->
        ...
        let jump_past_else_if_goto, prog =
          transform_stmt (then_start, prog) s1
        in
        ...
        let _, prog =
          insert_in_program (jump_past_if, prog) goto_ifstart_pc
            (Goto start_of_if)
        in
    ...
    | While.While (b, s) -> 
        ...
        let jump_over_while prog pc =
          insert_in_program prog ifpc (IfGoto (v, op, pc))
        in
    ...
    | While.Let _ ->
        failwith
          "Let translation to while3addr is not implemented and not part of \
           this assignment; try While code without Let instead"
  in
